...
struct MatchCandidate {
    int idxA;
    int idxB;
    float alignmentError; // Projection onto tangent (lower is better)
    float distSq;         // Euclidean distance squared (lower is better)
...
void coupledSmoothProj(std::vector<Point>& contourA, std::vector<Point>& contourB) {
    
    std::vector<std::vector<Point>> contours = {contourA, contourB};
    
    // 1. Build Spatial Grid (unchanged)
    std::map<Coord, std::vector<PointID>> grid;
    for (int c = 0; c < 2; ++c) {
        for (int p = 0; p < (int)contours[c].size(); ++p) {
            grid[{ (int)std::round(contours[c][p].x), (int)std::round(contours[c][p].y) }].push_back({c, p});
    ...
    std::vector<std::vector<Point>> targetPos = {contourA, contourB};
    float pairRadiusSq = 2.5f * 2.5f; // Slightly increased for projection context
    ...
    for (int c = 0; c < 2; ++c) {
        for (int p = 1; p < (int)contours[c].size() - 1; ++p) {
            ...
            // --- STEP A: My Quadratic Target ---
            Point myTarget = getQuadraticTarget(contours[c], p);

            // --- STEP B: Find Projection on Partner ---
            Point sumProjections = {0,0};
            int projCount = 0;

            int gx = (int)std::round(myPos.x);
            int gy = (int)std::round(myPos.y);

            // 1. Collect candidate neighbors
            ...
            for (int dy = -2; dy <= 2; ++dy) {
                for (int dx = -2; dx <= 2; ++dx) {
                    ...
                    for (const auto& nb : it->second) {
                        if (nb.cIdx == c) continue; // Ignore self
                        if (distSq(myPos, contours[nb.cIdx][nb.pIdx]) < pairRadiusSq) {
            ...
            // 2. Project onto segments formed by neighbors
            // Instead of averaging vertices, we find the closest spot on the geometry
            if (!neighbors.empty()) {
                float bestDist = 100000.0f;
                Point bestProj = {0,0};
                bool foundProj = false;

                const auto& otherContour = contours[neighbors[0].cIdx]; // Assume 1 partner contour

                // Check segments connected to neighbors
                for (const auto& nb : neighbors) {
                    int idx = nb.pIdx;
                    
                    // Segment: idx -> idx+1
                    if (idx < (int)otherContour.size() - 1) {
                        std::pair<Point, float> res = getClosestPointOnSegment(myTarget, otherContour[idx], otherContour[idx+1]);
                        // float d = distSq(myTarget, proj);
                        Point proj = res.first;
                        float d = res.second;
                        if (d < bestDist) { bestDist = d; bestProj = proj; foundProj = true; }
                    }
                    // Segment: idx-1 -> idx
                    if (idx > 0) {
                        std::pair<Point, float> res = getClosestPointOnSegment(myTarget, otherContour[idx-1], otherContour[idx]);
                        // float d = distSq(myTarget, proj);
                        Point proj = res.first;
                        float d = res.second;
                        if (d < bestDist) { bestDist = d; bestProj = proj; foundProj = true; }
                ...
                if (foundProj) {
                    sumProjections = bestProj;
                    projCount = 1; // We treat the "best projection" as the single authoritative partner target
                }
            }

            // --- STEP C: Consensus ---
            if (projCount > 0) {
                // Average "My Quadratic Shape" with "My Projection on Their Shape"
                // This snaps me to their curve while preserving my smoothness
                targetPos[c][p].x = (myTarget.x + sumProjections.x) / 2.0f;
                targetPos[c][p].y = (myTarget.y + sumProjections.y) / 2.0f;
            } else {
                targetPos[c][p] = myTarget;
            }
        }
...
// --- CORE LOGIC: Local Regression Projection ---
Point getMLSSnap(Point p, const std::vector<Point>& neighbors) {
    if (neighbors.empty()) return p;

    // 1. Calculate Centroid (Weighted Average Position)
    float sumX = 0, sumY = 0;
    float weightSum = 0;

    for (const auto& n : neighbors) {
        // Weight by distance (Gaussian kernel) gives smoother results than uniform
        float d2 = distSq(p, n);
        float w = std::exp(-d2 / 2.0f); // Sigma ~ 1.4
    ...
    float centerX = sumX / weightSum;
    float centerY = sumY / weightSum;

    // 2. Calculate Covariance Matrix to find the "Tangent" direction
    // (This finds the best-fit line direction through the cloud)
    float covXX = 0, covXY = 0, covYY = 0;
    
    for (const auto& n : neighbors) {
        float d2 = distSq(p, n);
        float w = std::exp(-d2 / 2.0f);
    ...
    // 3. Eigen Decomposition for 2x2 Matrix
    // The eigenvector with the LARGEST eigenvalue is the Tangent (direction of the curve).
    // The eigenvector with the SMALLEST eigenvalue is the Normal.
    // We want to project P onto the line passing through Center along the Tangent.
    
    // Calculate angle of dominant eigenvector
    float theta = 0.5f * std::atan2(2.0f * covXY, covXX - covYY);
    float tanX = std::cosf(theta);
    float tanY = std::sinf(theta);
    
    // 4. Project P onto the line defined by (Center, Tangent)
    // Line eq: Center + t * Tangent
    // Vector from Center to P
    float vcx = p.x - centerX;
    float vcy = p.y - centerY;
    
    // Dot product to find distance along line
    float t = vcx * tanX + vcy * tanY;
    
    // The point on the regression line
    return { centerX + t * tanX, centerY + t * tanY };
}

void stitchMLS(std::vector<Point>& vecA, std::vector<Point>& vecB, int iterations) {
    // Parameters
    float searchRadius = 2.5; // Radius of the "Sanding Block" (2.5 pixels covers the 3x3+ gap)
    float rSq = searchRadius * searchRadius;

    // Run a few iterations to let the points converge to a single smooth line
    for (int iter = 0; iter < iterations; ++iter) {
        
        // 1. Build Spatial Grid with ALL points (A and B combined)
        std::map<Coord, std::vector<Point>> grid;
        auto addToGrid = [&](const std::vector<Point>& vec) {
            for (const auto& p : vec) {
                grid[{ (int)std::round(p.x), (int)std::round(p.y) }].push_back(p);
            }
        };
        addToGrid(vecA);
        addToGrid(vecB);
        ...
        for (size_t i = 0; i < vecA.size(); ++i) {
            ...
            int gx = (int)std::round(vecA[i].x);
            int gy = (int)std::round(vecA[i].y);
            
            // Gather neighbors from 5x5 grid (radius 2)
            for (int dy = -2; dy <= 2; ++dy) {
                for (int dx = -2; dx <= 2; ++dx) {
                    auto it = grid.find({gx + dx, gy + dy});
                    if (it != grid.end()) {
                        for(const auto& n : it->second) {
                            if (distSq(vecA[i], n) <= rSq) {
            ...
            // Only adjust if we have enough context (e.g., > 2 points)
            // and if we actually see neighbors from the "other" side (heuristic check)
            if (neighbors.size() >= 3) {
                newA[i] = getMLSSnap(vecA[i], neighbors);
            }
        }

        // 3. Adjust Vector B (Same logic)
        std::vector<Point> newB = vecB;
        for (size_t i = 0; i < vecB.size(); ++i) {
            std::vector<Point> neighbors;
            int gx = (int)std::round(vecB[i].x);
            int gy = (int)std::round(vecB[i].y);
            ...
            if (neighbors.size() >= 3) {
                newB[i] = getMLSSnap(vecB[i], neighbors);
            }
        }
    ...
    std::cout << "MLS Stitching complete.\n";
}
...
void tightlyPackContours(std::vector<std::vector<Point>>& contours, int iterations) {
    
    float searchRadius = 1.5f; // Look for neighbors within ~2.5 pixels
    float radiusSq = searchRadius * searchRadius;

    // We iterate to allow the system to settle (relax)
    for (int iter = 0; iter < iterations; ++iter) {
        
        // 1. Build Spatial Map (Rebuilt every iteration as points move)
        // Key: Grid Coordinate -> Value: List of points in that cell
        std::map<Coord, std::vector<PointID>> grid;
        ...
        for (int c = 0; c < (int)contours.size(); ++c) {
            for (int p = 0; p < (int)contours[c].size(); ++p) {
                ...
                grid[{gx, gy}].push_back({c, p});
            }
        }

        // We store new positions in a temporary buffer to update synchronously
        std::vector<std::vector<Point>> nextContours = contours;

        // 2. Calculate Forces for every point
        for (int c = 0; c < (int)contours.size(); ++c) {
            for (int p = 0; p < (int)contours[c].size(); ++p) {
                ...
                // Check 5x5 neighborhood (Radius 2)
                for (int dy = -2; dy <= 2; ++dy) {
                    for (int dx = -2; dx <= 2; ++dx) {
                        auto it = grid.find({gx + dx, gy + dy});
                        if (it == grid.end()) continue;
                        ...
                        for (const auto& neighbor : it->second) {
                            // Don't interact with self or own contour 
                            // (We only want to snap to *other* vectors)
                            if (neighbor.cIdx == c) continue; 

                            const auto& otherContour = contours[neighbor.cIdx];
                            int idxB = neighbor.pIdx;

                            // We check the segments connected to this neighbor
                            // Segment 1: neighbor -> neighbor+1
                            if (idxB < (int)otherContour.size() - 1) {
                                Point target = getClosestPoint(currentPos, otherContour[idxB], otherContour[idxB+1]);
                                if (distSq(currentPos, target) < radiusSq) {
                                    sumTargets.x += target.x;
                                    sumTargets.y += target.y;
                                    matchCount++;
                                }
                            }
                            // Segment 2: neighbor-1 -> neighbor
                            if (idxB > 0) {
                                Point target = getClosestPoint(currentPos, otherContour[idxB-1], otherContour[idxB]);
                                if (distSq(currentPos, target) < radiusSq) {
                                    sumTargets.x += target.x;
                                    sumTargets.y += target.y;
                                    matchCount++;
                ...
                // 3. Apply Forces (Averaging)
                if (matchCount > 0) {
                    // The new position is an average of:
                    // 1. The point's current position (Inertia/Stiffness)
                    // 2. All the target projections found
                    
                    // "Stiffness" determines how easily the point moves. 
                    // Higher stiffness = smoother curve, but slower gap closing.
                    float stiffness = 10.0f; 
                    
                    float avgX = (sumTargets.x + currentPos.x * stiffness) / (matchCount + stiffness);
                    float avgY = (sumTargets.y + currentPos.y * stiffness) / (matchCount + stiffness);
                    
                    nextContours[c][p] = {avgX, avgY};
                }
            }
        }
    ...
    std::cout << "Packed " << contours.size() << " vectors over " << iterations << " iterations.\n";
}

// Gaussian Weight Function
float getWeight(float d2, float sigmaSq) {
    return std::exp(-d2 / (2.0f * sigmaSq));
}

// --- Core Logic: Adaptive Projection ---
Point calculateConsensus(Point p, const std::vector<Point>& cloud) {
    if (cloud.empty()) return p;
    ...
    for (const auto& n : cloud) {
        float w = getWeight(distSq(p, n), sigmaSq);
        sumX += n.x * w;
        sumY += n.y * w;
        weightSum += w;
    }
    
    if (weightSum == 0) return p;
    float centerX = sumX / weightSum;
    float centerY = sumY / weightSum;

    // 2. Calculate Covariance Matrix (2x2)
    // | xx  xy |
    // | xy  yy |
    float covXX = 0, covXY = 0, covYY = 0;
    for (const auto& n : cloud) {
    ...
    // 3. Eigen Analysis
    // We solve the characteristic equation: lambda^2 - Trace*lambda + Det = 0
    float trace = covXX + covYY;
    float det = covXX * covYY - covXY * covXY;
    float term = std::sqrt(std::max(0.0f, trace * trace / 4.0f - det));
    
    float lambda1 = trace / 2.0f + term; // Largest Eigenvalue (Variance along tangent)
    float lambda2 = trace / 2.0f - term; // Smallest Eigenvalue (Variance along normal)

    // 4. Linearity Check
    // If lambda1 >> lambda2, the cloud is a line (Edge).
    // If lambda1 ~ lambda2, the cloud is a blob (Triple Junction).
    float linearity = (lambda1 - lambda2) / (lambda1 + lambda2 + 1e-9);

    if (linearity > 0.6f) { 
        // --- CASE A: EDGE (Snap to Regression Line) ---
        // Eigenvector for lambda1 is the tangent
        float theta = 0.5f * std::atan2(2.0f * covXY, covXX - covYY);
        float tanX = std::cos(theta);
        float tanY = std::sin(theta);

        // Project Center->P onto Tangent
        float vcx = p.x - centerX;
        float vcy = p.y - centerY;
        float t = vcx * tanX + vcy * tanY;
        ...
        // --- CASE B: JUNCTION (Snap to Centroid) ---
        // We cannot define a line at a Y-junction, so we just pull 
        // strictly towards the weighted center of mass.
        return { centerX, centerY };
    }
}

void packTightlyMLS(std::vector<std::vector<Point>>& contours, int iterations) {
    float searchRadius = 1.5f; 
    float rSq = searchRadius * searchRadius;

    for (int iter = 0; iter < iterations; ++iter) {
        
        // 1. Build Global Spatial Map
        // We include points from ALL contours in this map.
        std::map<Coord, std::vector<Point>> globalGrid;
        for (const auto& contour : contours) {
            for (const auto& p : contour) {
                globalGrid[{ (int)std::round(p.x), (int)std::round(p.y) }].push_back(p);
            }
        }
        ...
        for (size_t c = 0; c < contours.size(); ++c) {
            for (size_t i = 0; i < contours[c].size(); ++i) {
                Point p = contours[c][i];
                int gx = (int)std::round(p.x);
                int gy = (int)std::round(p.y);

                // Gather Neighbors (Global Cloud)
                std::vector<Point> cloud;
                for (int dy = -3; dy <= 3; ++dy) {
                    for (int dx = -3; dx <= 3; ++dx) {
                        auto it = globalGrid.find({gx + dx, gy + dy});
                        if (it != globalGrid.end()) {
                            for (const auto& n : it->second) {
                                if (distSq(p, n) <= rSq) {
                                    cloud.push_back(n);
                                }
                ...
                // If isolated, don't move
                if (cloud.size() < 3) continue;

                // 3. Move point to Consensus
                nextContours[c][i] = calculateConsensus(p, cloud);
            }
        }
    ...
    std::cout << "Packed " << contours.size() << " contours with Adaptive MLS.\n";
}
...
void coupledSmooth(std::vector<std::vector<Point>>& contours, 
    ...
    for (int c = 0; c < (int)contours.size(); ++c) {
        for (int p = 1; p < (int)contours[c].size() - 1; ++p) {
                    ...
                    for (const auto& neighbor : it->second) {
                        ...
                        if (distSq(myPos, otherPos) < pairRadiusSq) {
                            ...
                            if (op > 0 && op < (int)otherContour.size() - 1 && 
                                ...
                                Point oTarget;
                                oTarget.x = 0.25f * oPrev.x + 0.5f * otherPos.x + 0.25f * oNext.x;
                                oTarget.y = 0.25f * oPrev.y + 0.5f * otherPos.y + 0.25f * oNext.y;
            ...
            if (partnerCount > 0) {
                float avgTx = (myTarget.x + sumPartnerTargets.x) / (1.0f + partnerCount);
                float avgTy = (myTarget.y + sumPartnerTargets.y) / (1.0f + partnerCount);
...
// --- Main Solver ---
void packWithBoundaryConstraints(std::vector<std::vector<Point>>& contours, Rect bounds, int iterations) {
