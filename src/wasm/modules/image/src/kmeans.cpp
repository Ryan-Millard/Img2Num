...
void _process_dist_per_centroid(
    const ImageLib::Image<PixelT> &pixels,
  ...
  for (int32_t iter{0}; iter < max_iter; ++iter) {
    ...
    if (nthreads > 1) {
      for (int i = 0; i < nthreads; ++i) {
        ...
        switch (color_space) {
          case COLOR_SPACE_OPTION_RGB : {
            threads.emplace_back(_process_dist_per_centroid<ImageLib::RGBAPixel<float>>,
                             std::cref(pixels), std::cref(centroids),
                             std::ref(distances), start_c, end_c);
            break;
          }
          case COLOR_SPACE_OPTION_CIELAB : {
            threads.emplace_back(_process_dist_per_centroid<ImageLib::LABAPixel<float>>,
                             std::cref(lab), std::cref(centroids_lab),
                             std::ref(distances), start_c, end_c);
            break;
          }
        }
      ...
      for (int i = 0; i < nthreads; ++i) {
        ...
        switch (color_space) {
          case COLOR_SPACE_OPTION_RGB : {
            threads.emplace_back(_apply_labels<ImageLib::RGBAPixel<float>>, std::cref(pixels),
          ...
          case COLOR_SPACE_OPTION_CIELAB : {
            threads.emplace_back(_apply_labels<ImageLib::LABAPixel<float>>, std::cref(lab),
                             std::cref(distances), std::ref(labels),
                             start_pixel, end_pixel, k,
                             std::ref(changed_atomic));
            break;
