...
// Sets a pixel (no bounds check needed if logic is correct, but added for safety)
inline void setPixel(std::vector<uint8_t>& img, int w, int x, int y, uint8_t val) {
    img[y * w + x] = val;
}
...
void skeletonize(std::vector<uint8_t>& img, int width, int height) {
    ...
    // Relative coordinates for the 8 neighbors in specific order: P2, P3, ..., P9, P2
    // P9 P2 P3
    // P8 P1 P4
    // P7 P6 P5
    ...
    while (pixelRemoved) {
        ...
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                if (getPixel(img, width, height, x, y) != 1) continue;
                ...
                if (p[7] == 0 && p[0] == 1) A++; // Wrap around P9 -> P2
        ...
        if (!markers.empty()) {
            pixelRemoved = true;
            for (auto& pt : markers) setPixel(img, width, pt.first, pt.second, 0);
        }
...
std::vector<uint8_t> analyzeTopology(const std::vector<uint8_t>& skel, int w, int h) {
...
void smoothChainGaussian(std::vector<Point>& chain) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (!isClosed && (i < 2 || i >= n - 2)) continue;
...
void ramerDouglasPeucker(const std::vector<Point>& points, float epsilon, std::vector<Point>& out) {
    if (points.size() < 2) { out = points; return; }
...
std::vector<std::vector<Point>> computeSubpixelCenterlines(const std::vector<uint8_t>& skel, int w, int h) {
    ...
    */
    ...
    auto trace = [&](int sx, int sy, int firstNx = -1, int firstNy = -1) {
        ...
        if (isJunction(sy*w+sx)) {
            chain.push_back({(float)sx, (float)sy}); // junctionCentroids[junctionMap[sy*w+sx]]);
        } else {
        ...
        if (firstNx != -1) {
            cx = firstNx; cy = firstNy;
            if (isJunction(cy*w+cx)) {
                chain.push_back({(float)cx, (float)cy}); // junctionCentroids[junctionMap[cy*w+cx]]);
        ...
        while (true) {
            ...
            for (int i=0; i<8; ++i) {
                ...
                int nIdx = ny*w+nx;
                if (!getPixel(skel, w, h, nx, ny)) continue;

                // if (isJunction(nIdx)) { bestDir = i; break; } // Hit Junction
                // if (!visited[nIdx] && topology[nIdx] != 3) { bestDir = i; break; } // Hit Line
                if (isJunction(nIdx)){
                    choices[i] = 3;
                    continue;
                }
            ...
            if (bestDir == -1) break; 
            cx += dx[bestDir]; cy += dy[bestDir];
            int cIdx = cy*w+cx;

            if (isJunction(cIdx)) {
                chain.push_back({(float)cx, (float)cy}); // junctionCentroids[junctionMap[cIdx]]);
                break;
            } else {
                chain.push_back({(float)cx, (float)cy});
                visited[cIdx] = true;
    ...
    for (int y=0; y<h; ++y) {
        for (int x=0; x<w; ++x) {
            if (topology[y*w+x] == 2 && !visited[y*w+x]) trace(x, y);
        }
    }
        ...
        for (int x=0; x<w; ++x) {
            if (isJunction(y*w+x)) {
                for (int i=0; i<8; ++i) {
                    ...
                    if (getPixel(skel,w,h,nx,ny) && topology[ny*w+nx] == 1 && !visited[ny*w+nx]) {
                        trace(x, y, nx, ny);
                    }
                }
            }
    ...
    for (int y=0; y<h; ++y) {
        for (int x=0; x<w; ++x) {
            if (getPixel(skel,w,h,x,y) && !visited[y*w+x] && !isJunction(y*w+x)) {
                trace(x, y);
                if (!chains.empty()) {
...
std::vector<std::vector<Point>> getAtomicRegions(const std::vector<std::vector<Point>>& chains) {
    ...
    auto getNodeID = [&](Point p) -> int {
        for (int i=0; i<nodes.size(); ++i) 
            if (distSq(p, nodes[i].p) < mergeThreshold) return i;
    ...
    while(changed) {
        ...
        for(int i=0; i<nodes.size(); ++i) {
            ...
            for(const auto& e : nodes[i].adj) {
                if(active[e.targetNode]) deg++;
                if(e.targetNode == i) selfLoop = true;
            }
            if(deg <= 1 && !selfLoop) { active[i] = false; changed = true; }
        }
    }
    ...
    for (auto& node : nodes) {
        ...
        std::sort(node.adj.begin(), node.adj.end(), [&](const GraphEdge& a, const GraphEdge& b) {
            // Determine geometry of the outgoing edge to calculate angle
            // If reverse=false: starts at chain.front (Node U), goes to chain[1]
            // If reverse=true:  starts at chain.back (Node V), goes to chain[N-2]
            Point p1 = node.p;
            Point p2;
    ...
    for (int i = 0; i < nodes.size(); ++i) {
        ...
        for (const auto& startEdge : nodes[i].adj) {
            if (!active[startEdge.targetNode]) continue;
            
            // If we already traversed this specific half-edge, skip
            if (visitedEdges.count({i, startEdge.targetNode, startEdge.chainIdx})) continue;
            ...
            while (safety++ < 10000) { // Safety break
                ...
                if (nextEdge.reverse) {
                    for (auto it = c.rbegin(); it != c.rend() - 1; ++it) polygon.push_back(*it);
                } else {
                    for (auto it = c.begin(); it != c.end() - 1; ++it) polygon.push_back(*it);
                }
                ...
                // We need to find the edge going back to 'prev' in curr's sorted list...
                ...
                for (int k = 0; k < adj.size(); ++k) {
                    if (adj[k].targetNode == prev && adj[k].chainIdx == nextEdge.chainIdx) {
                        incomingIndex = k;
                        break;
                    }
                }

                if (incomingIndex == -1) break; // Should not happen in valid graph

                // "Left Turn" in screen coords (Y-down) usually corresponds to 
                // picking the PREVIOUS edge in angular sort order, or NEXT depending on sort direction.
                // Standard CCW traversal on Y-Down:
                // If angles sorted -PI to +PI:
                // The "sharpest left" is the neighbor immediately cyclic-before the incoming edge.
                int outgoingIndex = (incomingIndex - 1 + adj.size()) % adj.size();
                
                nextEdge = adj[outgoingIndex];
                
                // If the "next" edge is dead (pruned), we are stuck (shouldn't happen with pruning)
                if (!active[nextEdge.targetNode]) break; 
            }
            ...
            if (loopClosed && polygon.size() > 2) {
                ...
                // Note: If your coords are standard Cartesian (Y-Up), invert this check.
