...
inline bool sameColor(const uint8_t* img, int w, int h, int x1, int y1, int x2, int y2) {
    int i1 = idx(x1,y1,w)*4;
    int i2 = idx(x2,y2,w)*4;
    return img[i1]==img[i2] && img[i1+1]==img[i2+1] && img[i1+2]==img[i2+2] && img[i1+3]==img[i2+3];
}
...
// TODO: check for gaps inside regions - its possible their dimensions are fine, but inner gaps reduce effective
// width and height
void mergeSmallRegionsInPlace(uint8_t* pixels, int width, int height, int minArea, int minWidth, int minHeight) {
            ...
            while(!q.empty()) {
                ...
                for(auto& d : dirs) {
                    ...
                    if(nx>=0 && nx<width && ny>=0 && ny<height) {
                        if(labels[idx(nx,ny,width)] == -1 && sameColor(pixels,width,height,p.x,p.y,nx,ny)) {
    ...
    for(int y=0; y<height; ++y) {
        for(int x=0; x<width; ++x) {
            ...
            if(regions[l].isBigEnough(minArea, minWidth, minHeight)) continue;
