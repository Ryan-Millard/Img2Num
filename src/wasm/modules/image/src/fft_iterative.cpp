...
void bit_reverse_permute(std::vector<cd> &a) {
  ...
  int log_n = 0; // Base 2
  while ((1u << log_n) < N)
    ++log_n; // Set log_n to the exponent of 2 such that N = 2^log_n
  ...
  for (size_t i = 0; i < N; ++i) {
    ...
    if (i < rev)
      std::swap(a[i], a[rev]); // Avoid swapping twice
  }
}
...
void iterative_fft(std::vector<cd> &a, bool inverse) {
  ...
  const double sign =
      inverse ? +1.0
              : -1.0; // -1 for forward (engineering convention), +1 for inverse
...
void iterative_fft_2d(std::vector<cd> &data, size_t width, size_t height,
                      bool inverse) {
  ...
  // FFT rows
  for (size_t y = 0; y < height; y++) {
    std::copy(data.begin() + y * width, data.begin() + (y + 1) * width,
              temp_row.begin());
