...
namespace ImageLib {
	...
	template<typename NumberT>
	inline const PixelConverter<RGBPixel<NumberT> (*)(const uint8_t*)> RGB_CONVERTER { convertRGB,  3 }; // 3 bytes per pixel
	template<typename NumberT>
	inline const PixelConverter<RGBAPixel<NumberT>(*)(const uint8_t*)> RGBA_CONVERTER{ convertRGBA, 4 }; // 4 bytes per pixel
