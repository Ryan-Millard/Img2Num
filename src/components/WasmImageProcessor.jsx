...
const WasmImageProcessor = () => {
	...
	useEffect(() => {
		console.log('editedSvg state changed:', editedSvg ? 'SVG set' : 'SVG cleared');
	}, [editedSvg]);
	...
	const processImage = async () => {
		...
		try {
			...
			console.log('About to convert to SVG...');
			const svgString = await uint8ClampedArrayToSVG({ pixels: newPixels, width, height });
			console.log('SVG conversion result:', svgString ? 'Success' : 'Failed');

			if (svgString) {
				console.log('Setting SVG state...');
