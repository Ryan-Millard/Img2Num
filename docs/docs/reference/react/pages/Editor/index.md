...
- A **Color mode** that hides fills until a shape is tapped/clicked (reveal is one-way; shapes stay colored once revealed)
...
- `isColorMode` (boolean)
  - `true`: shapes are “hidden” (filled white) until clicked
...
- If the user **did not drag** and `isColorMode` is enabled:
  ...
  3.  Add `styles.coloredRegion` on that element

Once a shape is revealed it stays colored; repeated taps do not re-hide or toggle it off.
...
- When the user taps/clicks, the page adds `.coloredRegion` on that SVG element
