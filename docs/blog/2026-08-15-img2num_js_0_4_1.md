---
title: "Img2Num JS 0.4.1: One Package, Five Purpose-Built Bundles"
slug: img2num_js_0_4_1
authors: [ryan-millard]
tags: [release, javascript, wasm]
---

Img2Num's [JavaScript package v0.4.1](https://www.npmjs.com/package/img2num) is out on npm, and it's the biggest change to
...
:::warning[Why 0.4.1 and not 0.4.0?]

0.4.0 shipped everything described below - and a bug. Its browser build published a wasm URL
that bundlers couldn't statically detect, so bundled apps (Vite, webpack, Rollup) hit a 404
on `img2num.wasm` at runtime. Our own React example app caught it within a day.
0.4.0 is now [deprecated on npm](https://www.npmjs.com/package/img2num/v/0.4.0);
install `>=0.4.1` and you get the release as intended, with a build-time guard that makes
this class of bug impossible to publish again.
...
[0.4.1](https://github.com/Ryan-Millard/Img2Num/releases/tag/packages-js-v0.4.1)
...
0.4.1 mangles the URL during our build and restores the bundler-detectable literal in the published output,
...
If you're moving from an earlier version straight to [v0.4.1](https://www.npmjs.com/package/img2num),
...
[v0.4.1](https://www.npmjs.com/package/img2num) is what that change unlocked:
...
- The `html-js` example was rebuilt as a single shared template compiled into three self-contained variants -
  <a href="/example-apps/esm/" target="_blank">
    ESM
  </a>,<a href="/example-apps/iife/" target="_blank">
    IIFE
  </a>, and
  <a href="/example-apps/umd/" target="_blank">
    UMD
  </a> - each deployable as plain static files with no build tooling at runtime.
...
They're also our first line of defense - the React example is what caught the 0.4.0 bug before almost anyone could hit it.
...
<script src="https://cdn.jsdelivr.net/npm/img2num@0.4.1/dist/standalone/img2num.iife.js"></script>
```

(Pin the version - your future self will thank you. Ours certainly did this week.)
