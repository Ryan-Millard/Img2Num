...
constexpr int32_t OUTSIDE = -2147483647;  // image exterior label
...
void reverse_curve(std::vector<QuadBezier> &c) {
    ...
    for (QuadBezier &q : c) std::swap(q.p0, q.p2);
}
...
std::unordered_map<int32_t, std::vector<std::vector<QuadBezier>>> build_shared_loops(
    ...
    auto cidx = [&](int cx, int cy) { return cy * W1 + cx; };
    auto cx_of = [&](int idx) { return idx % W1; };
...
<<<<<<< HEAD
    ...
    auto ckey = [](int a, int b) { return std::make_pair(std::min(a, b), std::max(a, b)); };
...
<<<<<<< HEAD
    for (auto &kv : adj) {
        ...
        for (int nb : kv.second)
            if (!crack_edge.count(ckey(kv.first, nb))) store_edge(walk_edge(kv.first, nb), false);
    }
...
<<<<<<< HEAD
            ...
            if (L(x, y - 1) != r) dir[c00].push_back(c10);  // top  ->
            if (L(x + 1, y) != r) dir[c10].push_back(c11);  // right v
            if (L(x, y + 1) != r) dir[c11].push_back(c01);  // bottom <-
            if (L(x - 1, y) != r) dir[c01].push_back(c00);  // left  ^
        }
...
<<<<<<< HEAD
                    ...
                    for (const QuadBezier &q : seg) curve.push_back(q);
                    i = m;
                } else {
                    bool fwd = (from == e.a);
                    if (!fwd) reverse_curve(seg);
