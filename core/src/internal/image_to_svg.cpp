...
std::string image_to_svg(const uint8_t *data, const int width, const int height, const ImageToSvgConfig& config) {
    ...
    bilateral_filter(img_data.data(), width, height, config.bilateral_filter.sigma_spatial, config.bilateral_filter.sigma_range, config.color_space);
    kmeans(img_data.data(), out_data.data(), out_labels.data(), width, height,config.kmeans.k, config.kmeans.max_iter,
           config.color_space);
