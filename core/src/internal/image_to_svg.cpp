...
std::string image_to_svg(
    ...
    std::memcpy(img_data.data(), data, static_cast<size_t>(width) * static_cast<size_t>(height) * 4);
    bilateral_filter(img_data.data(), width, height, sigma_spatial, sigma_range, color_space);
    kmeans(img_data.data(), out_data.data(), out_labels.data(), width, height, k, max_iter, color_space);
